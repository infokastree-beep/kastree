...
# Keep in sync with frontend RiskFlagsPanel empty state after a clean analysis run.
RISK_FLAGS_EMPTY_MESSAGE = "No risk flags were raised for this trial balance."
WATERMARK_TEXT = "Generated by Kastree"
...
def build_pdf(
    ...
    *,
    ...
    """Board-ready PDF via WeasyPrint. Free tier gets a Kastree watermark."""
...
def _pdf_pack_sections(
    ...
    if not risk_rows:
        risk_body = f"<p>{_html_escape(RISK_FLAGS_EMPTY_MESSAGE)}</p>"
    else:
        risk_body = (
    ...
    sections.append(
        (
            "Risk Flags",
            f"<section><h2>Risk Flags</h2>{risk_body}</section>",
        )
    )
...
def _write_risk_sheet(
    ...
    row = _write_disclaimer(ws, row)
    if not flags:
        # Match dashboard RiskFlagsPanel empty copy — no bare header-only table.
        ws.cell(row, 1, RISK_FLAGS_EMPTY_MESSAGE)
        _autosize(ws, 1)
        return
